...
object AnimatedSniff {
    ...
    fun isApng(header: ByteArray, length: Int): Boolean {
    ...
    fun isAnimatedWebP(header: ByteArray, length: Int): Boolean =
        indexOfMagic(header, length, "ANIM") >= 0

    fun readHeader(path: String, maxBytes: Int = SNIFF_BYTES): Pair<ByteArray, Int>? =
        try {
    ...
    fun indexOfMagic(buf: ByteArray, length: Int, magic: String): Int {
